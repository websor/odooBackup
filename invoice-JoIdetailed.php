...
require_once('Class/Journal_Item.class.php');
 ...
 */
 ...
 /**
* Call to the database to get the journal entrie
* 
*/ 
$newInvoice = new Journal_Entries();

$query_user = "select * from journal_entries where number = '$inv';";
$result_User = mysqli_query($conection,$query_user);
while($row_user = mysqli_fetch_assoc($result_User))
{ 
    $newInvoice = new Journal_Entries(); 
    $newInvoice->setCreated_on($row_user['created_on']);
    $newInvoice->setDate($row_user['date']);
    $newInvoice->setNumber($row_user['number']);
    $newInvoice->setCustomer($row_user['customer']);
    $newInvoice->setReference($row_user['reference']);
    $newInvoice->setJournal($row_user['journal']);
    $newInvoice->setStatus($row_user['status']); 
    $newInvoice->setAmount($row_user['amount']); 

}


/**
* Call to the database to get all the journal items
* 
*/ 
$invoiceLines = array();
$query_user_line = "select * from journal_items where number = '$inv';";
$result_User_line = mysqli_query($conection,$query_user_line);
while($row_user = mysqli_fetch_assoc($result_User_line))
{ 
    $newInvoiceLine = new Journal_Item();
    $newInvoiceLine->setCreated_on($row_user['created_on']);
    $newInvoiceLine->setDate($row_user['due_date']);
    $newInvoiceLine->setNumber($row_user['number']);
    $newInvoiceLine->setCustomer($row_user['customer']);
    $newInvoiceLine->setAccount($row_user['account']);
    $newInvoiceLine->setLable($row_user['label']);
    $newInvoiceLine->setReference($row_user['reference']); 
    $newInvoiceLine->setDebit($row_user['debit']); 
    $newInvoiceLine->setCredit($row_user['credit']); 
...
Page::invoiceJoIDetailed($typ, $newInvoice, $invoiceLines, $email, $type, $inv);
Page::footer2();
Page::endHead2();
